...
trait VersionRoutingMap {
  ...
  def versionRouter(version: Version): Option[Router] = map.get(version)
}

// Add routes corresponding to available versions...
case class VersionRoutingMapImpl @Inject() (defaultRouter: Router, versionRouters: Map[Version, Router])
  extends VersionRoutingMap {

  val map: Map[Version, Router] = versionRouters
}
