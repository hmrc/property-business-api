...
class RetrieveHistoricNonFhlUkPiePeriodSummaryController @Inject()(val authService: EnrolmentsAuthService,
  ...
  def handleRequest(nino: String, periodId: String): Action[AnyContent] =
    authorisedAction(nino).async { implicit request =>
      ...
      val rawData = RetrieveHistoricNonFhlUkPiePeriodSummaryRawData(nino, periodId)
      val result =
        for {
          ...
          vendorResponse <- EitherT.fromEither[Future](
            hateoasFactory
              .wrap(serviceResponse.responseData, RetrieveHistoricNonFhlUkPiePeriodSummaryHateoasData(nino, periodId))
