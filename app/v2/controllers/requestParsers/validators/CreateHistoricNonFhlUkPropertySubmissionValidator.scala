...
@Singleton
class CreateHistoricNonFhlUkPropertySubmissionValidator @Inject()(appConfig: AppConfig)
    extends Validator[CreateHistoricNonFhlUkPropertyPeriodSubmissionRawData] {

  lazy private val minTaxYear = appConfig.minimumTaxHistoric
  private lazy val maxTaxYear = appConfig.maximumTaxHistoric

  override def validate(data: CreateHistoricNonFhlUkPropertyPeriodSubmissionRawData): List[MtdError] = {
    (for {
      _    <- validatePathParams(data)
      body <- validateAndCheckNonEmptyOrRead[CreateHistoricNonFhlUkPropertyPeriodSubmissionRequestBody](data.body)
      _    <- validateBody(body)
    } yield ()).swap.getOrElse(Nil)
  }

  private def validatePathParams(data: CreateHistoricNonFhlUkPropertyPeriodSubmissionRawData): Either[List[MtdError], Unit] = {
    val errors =
  ...
  private def validateBody(body: CreateHistoricNonFhlUkPropertyPeriodSubmissionRequestBody): Either[List[MtdError], Unit] = {
