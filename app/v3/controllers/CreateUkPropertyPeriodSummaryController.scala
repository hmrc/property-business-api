...
import v3.controllers.requestParsers.CreateUkPropertyPeriodSummaryRequestParser
import v3.models.request.createUkPropertyPeriodSummary.CreateUkPropertyPeriodSummaryRawData
import v3.models.response.createUkPropertyPeriodSummary.CreateUkPropertyPeriodSummaryHateoasData
...
class CreateUkPropertyPeriodSummaryController @Inject() (val authService: EnrolmentsAuthService,
  ...
  private def auditHandler(rawData: CreateUkPropertyPeriodSummaryRawData, correlationId: String, request: UserRequest[JsValue]): AuditHandlerOld = {
    new AuditHandlerOld() {
      override def performAudit(userDetails: UserDetails, httpStatus: Int, response: Either[ErrorWrapper, Option[JsValue]], versionNumber: String)(
        ...
        response match {
          ...
          case Right(resp: Option[JsValue]) =>
            auditSubmission(
              GenericAuditDetailOld(request.userDetails, rawData, correlationId, AuditResponse(httpStatus = CREATED, response = Right(resp)))
            )
        }
      }
    }
