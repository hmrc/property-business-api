...
import v4.listHistoricNonFhlUkPropertyPeriodSummaries.model.response.{
...
@Singleton
class ListHistoricNonFhlUkPropertyPeriodSummariesConnector @Inject() (val http: HttpClient, val appConfig: AppConfig)
    extends BaseDownstreamConnector {

  def listPeriodSummaries(request: ListHistoricNonFhlUkPropertyPeriodSummariesRequestData)(implicit
      hc: HeaderCarrier,
      ec: ExecutionContext,
      correlationId: String): Future[DownstreamOutcome[ListHistoricNonFhlUkPropertyPeriodSummariesResponse]] = {
