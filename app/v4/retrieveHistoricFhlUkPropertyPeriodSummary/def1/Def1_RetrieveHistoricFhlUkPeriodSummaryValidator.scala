...
import v4.retrieveHistoricFhlUkPropertyPeriodSummary.model.request._


class Def1_RetrieveHistoricFhlUkPeriodSummaryValidator(nino: String, periodId: String, appConfig: AppConfig)
  extends Validator[RetrieveHistoricFhlUkPiePeriodSummaryRequestData] {

  private lazy val resolvePeriodId = new ResolvePeriodId(appConfig.minimumTaxYearHistoric, appConfig.maximumTaxYearHistoric)


  def validate: Validated[Seq[MtdError], RetrieveHistoricFhlUkPiePeriodSummaryRequestData] =
    (
      ResolveNino(nino),
      resolvePeriodId(periodId)
      ).mapN(Def1_RetrieveHistoricFhlUkPiePeriodSummaryRequestData)
