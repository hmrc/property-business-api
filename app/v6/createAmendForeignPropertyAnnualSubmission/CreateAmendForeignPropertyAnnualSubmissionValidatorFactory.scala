...
class CreateAmendForeignPropertyAnnualSubmissionValidatorFactory @Inject() (implicit config: PropertyBusinessConfig) {
  ...
  def validator(nino: String,
    ...
    CreateAmendForeignPropertyAnnualSubmissionSchema.schemaFor(taxYear) match {
      ...
      case Valid(Def3)     => new Def3_CreateAmendForeignPropertyAnnualSubmissionValidator(nino, businessId, taxYear, body)
