...
import v6.createAmendForeignPropertyAnnualSubmission.def3.model.request.def3_foreignProperty.{
  ...
  Def3_Create_Amend_StructuredBuildingAllowance
}

import java.time.LocalDate

class Def3_CreateAmendForeignPropertyAnnualSubmissionRulesValidator
    extends RulesValidator[Def3_CreateAmendForeignPropertyAnnualSubmissionRequestData] {
  ...
  private def validateForeignEntry(entry: Def3_Create_Amend_ForeignEntry, index: Int): Validated[Seq[MtdError], Unit] = {
    ...
    val validated = validatedNumberFields ++ List(validatedPropertyId, validatedPropertyIncomeAllowance, validatedAllowances) ++ validatedBuildings

    validated.sequence.andThen(_ => valid)
  }

  private def validateForeignAllowances(index: Int)(allowances: Def3_Create_Amend_ForeignAllowances): Validated[Seq[MtdError], Unit] = {
    allowances.propertyIncomeAllowance match {
      case None => valid
      case Some(_) =>
        allowances match {
          case Def3_Create_Amend_ForeignAllowances(None, None, None, None, Some(_), None) => valid
  ...
  private def validateStructuredBuildingAllowance(structuredBuildingAllowance: Def3_Create_Amend_StructuredBuildingAllowance,
                                                  index: Int,
