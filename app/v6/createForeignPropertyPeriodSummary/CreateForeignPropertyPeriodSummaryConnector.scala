...
@Singleton
class CreateForeignPropertyPeriodSummaryConnector @Inject() (val http: HttpClientV2, val appConfig: AppConfig) extends BaseDownstreamConnector {
  ...
  def createForeignProperty(request: CreateForeignPropertyPeriodSummaryRequestData)(implicit
    ...
    request match {
      ...
      case def2: Def2_CreateForeignPropertyPeriodSummaryRequestData =>
        import def2.*
        val downstreamUri = HipUri[CreateForeignPropertyPeriodSummaryResponse](
          s"itsa/income-tax/v1/${taxYear.asTysDownstream}/business/property/periodic/$nino/$businessId")
