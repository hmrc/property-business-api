...
class Def1_CreateAmendForeignPropertyAnnualSubmissionISpec extends IntegrationBaseSpec with Def1_Fixtures {
    ...
    "return an error according to spec" when {
      "validation error occurs" when {
        ...
        val input = List(
          ...
          ("AA123456A", "XAIS12345678910", "2023-24", JsObject.empty, BAD_REQUEST, RuleIncorrectOrEmptyBodyError),
          (
            "AA123456A",
            "XAIS12345678910",
            "2023-24",
          ...
          (
            ...
            BAD_REQUEST,
            RuleBothAllowancesSuppliedError.withPath("/foreignFhlEea/allowances")
          ),
          (
            "AA123456A",
            "XAIS12345678910",
      ...
      "downstream service error" when {
        def serviceErrorTest(downstreamStatus: Int, downstreamCode: String, expectedStatus: Int, expectedBody: MtdError): Unit = {
          s"downstream returns a code $downstreamCode error and status $downstreamStatus" in new TysHipTest {
            override def setupStubs(): StubMapping = {
              AuditStub.audit()
              AuthStub.authorised()
              MtdIdLookupStub.ninoFound(nino)
  ...
  private trait Test {
    ...
    private def mtdUri: String = s"/foreign/$nino/$businessId/annual/$taxYear"
