...
import api.routing.{Version3, Version4}
  ...
  "APIDefinition" when {
    ...
    "with multiple unique versions" should {
      "construct successfully" in {
        ...
        definition.versions should contain(versionV4)
      }
    }
  ...
  "Definition" when {
    ...
    "with complex nested APIDefinition" should {
      "maintain structure through serialization" in {
        ...
        deserialized.api.versions should have length 2
        deserialized.api.categories should have length 2
        deserialized.api.requiresTrust shouldBe Some(true)
      }
    }
  }
  ...
  "Json formats" when {
    "APIVersion format" should {
      ...
      "handle missing required fields" in {
        val format         = summon[OFormat[APIVersion]]
        val incompleteJson = Json.obj("version" -> "3.0") // missing status and endpointsEnabled
    ...
    "APIDefinition format" should {
      ...
      "handle missing required fields" in {
        val format         = summon[OFormat[APIDefinition]]
        val incompleteJson = Json.obj("name" -> "Test") // missing other required fields
        val result         = format.reads(incompleteJson)
        result shouldBe a[JsError]
      }
    }
    ...
    "Definition format" should {
      ...
      "handle missing required fields" in {
        val format         = summon[OFormat[Definition]]
        val incompleteJson = Json.obj() // missing api field
        val result         = format.reads(incompleteJson)
        result shouldBe a[JsError]
      }
    }
