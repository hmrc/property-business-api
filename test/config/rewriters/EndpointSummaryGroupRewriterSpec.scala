  ...
  "EndpointSummaryGroupRewriter" when {
    ...
    "rewrite" should {
      "return the rewritten summaries when the 'maybeTestOnly' helper is present" in {
        MockedAppConfig.endpointReleasedInProduction("4.0", "employment-expenses-create-and-amend") returns false
        MockedAppConfig.endpointReleasedInProduction("4.0", "employment-expenses-retrieve") returns true
        MockedAppConfig.endpointReleasedInProduction("4.0", "employment-expenses-delete") returns false

        val yaml =
          """
                  |put:
                  |  $ref: "./employment_expenses_create_and_amend.yaml"
                  |  summary: Create and Amend Employment Expenses{{#maybeTestOnly "4.0 employment-expenses-create-and-amend"}}{{/maybeTestOnly}}
                  |  security:
                  |    - User-Restricted:
                  ...
                  |  summary: Retrieve Employment Expenses{{#maybeTestOnly "4.0 employment-expenses-retrieve"}}{{/maybeTestOnly}}
                  |  security:
                  |    - User-Restricted:
                  ...
                  |  summary: Delete Employment Expenses{{#maybeTestOnly "4.0 employment-expenses-delete"}}{{/maybeTestOnly}}
                  |  security:
                  |    - User-Restricted:
                  |        - write:self-assessment
                  |
        ...
        val result = checkAndRewrite.rewrite(path = "/public/api/conf/4.0", filename = "employment_expenses.yaml", yaml)
      ...
      "return the unchanged yaml when the 'maybeTestOnly' is not present" in {
        ...
        val result = checkAndRewrite.rewrite("/public/api/conf/4.0", "employment_expenses.yaml", yaml)
