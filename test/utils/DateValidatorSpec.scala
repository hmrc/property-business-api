    ...
    "return error(s)" when {
      "a single date is provided" which {
        "is a fromDate with an invalid format" in {
          DateValidator.validateFromAndToDates(invalidFormatDate, None) shouldBe invalid(
            Seq(FromDateFormatError, RuleMissingSubmissionDatesError)
          )
        }
        "is a toDate with an invalid format" in {
          DateValidator.validateFromAndToDates(None, invalidFormatDate) shouldBe invalid(
            Seq(ToDateFormatError, RuleMissingSubmissionDatesError)
          )
        }
