  ...
  "running a validation" should {
    "return no errors" when {
      "a valid request is supplied" in {
        validator.validate(RetrieveHistoricFhlUkPiePeriodSummaryRawData(validNino, validPeriodId)) shouldBe Nil
      }
    }
    "return a validation error" when {
      "an invalid nino is supplied" in {
        validator.validate(RetrieveHistoricFhlUkPiePeriodSummaryRawData("Nino", validPeriodId)) shouldBe List(NinoFormatError)
      }
      "an invalid periodId format is supplied" in {
        validator.validate(RetrieveHistoricFhlUkPiePeriodSummaryRawData(validNino, "2017-04-06__2017-07-04")) shouldBe List(PeriodIdFormatError)
      }
      "a non-historic periodId is supplied" in {
        validator.validate(RetrieveHistoricFhlUkPiePeriodSummaryRawData(validNino, "2012-04-06_2012-07-04")) shouldBe List(PeriodIdFormatError)
      }
      "multiple format errors are made" in {
        validator.validate(RetrieveHistoricFhlUkPiePeriodSummaryRawData("Nino", "2012-04-06_2012-07-04")) shouldBe
