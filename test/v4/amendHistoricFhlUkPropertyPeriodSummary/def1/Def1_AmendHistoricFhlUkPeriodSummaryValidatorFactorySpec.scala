...
package v4.amendHistoricFhlUkPropertyPeriodSummary.def1
...
import v4.amendHistoricFhlUkPropertyPeriodSummary.AmendHistoricFhlUkPropertyPeriodSummaryValidatorFactory
import v4.amendHistoricFhlUkPropertyPeriodSummary.def1.model.request.{UkFhlPieExpenses, UkFhlPieIncome}
...
class Def1_AmendHistoricFhlUkPeriodSummaryValidatorFactorySpec extends UnitSpec with MockAppConfig with JsonErrorValidators {
  ...
  private val parsedUkFhlPieIncome = UkFhlPieIncome(Some(1123.45), Some(2134.53), Some(parsedUkPropertyIncomeRentARoom))
  ...
  private val parsedBody = Def1_AmendHistoricFhlUkPiePeriodSummaryRequestBody(
  ...
  "validator" should {
    "return the parsed domain object" when {
      "passed a valid request" in {
        setupMocks()
        val result: Either[ErrorWrapper, AmendHistoricFhlUkPiePeriodSummaryRequestData] =
          validator(validNino, validPeriodId, validBody).validateAndWrapResult()

        result shouldBe Right(Def1_AmendHistoricFhlUkPiePeriodSummaryRequestData(parsedNino, parsedPeriodId, parsedBody))
      }
      ...
      "passed a valid consolidated request" in {
        ...
        result shouldBe Right(Def1_AmendHistoricFhlUkPiePeriodSummaryRequestData(parsedNino, parsedPeriodId, parsedBodyConsolidated))
      }
    }
