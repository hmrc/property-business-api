...
package v4.controllers.amendForeignPropertyPeriodSummary.def2.model.request.foreignFhlEea

import play.api.libs.json.{JsValue, Json}
import support.UnitSpec
import v4.controllers.amendForeignPropertyPeriodSummary.def2.model.request.def2_foreignFhlEea.Def2_AmendForeignFhlEeaExpenses

class Def2_AmendForeignFhlEeaExpensesSpec extends UnitSpec {
  ...
  val model: Def2_AmendForeignFhlEeaExpenses = Def2_AmendForeignFhlEeaExpenses(
  ...
  "reads" should {
    "read from JSON" when {
      "valid JSON is provided" in {
        mtdJson.as[Def2_AmendForeignFhlEeaExpenses] shouldBe model
      }
    }
  }
