...
class Def1_Create_CreateForeignNonFhlPropertyEntrySpec extends UnitSpec {
  ...
  private val model = Def1_Create_CreateForeignNonFhlPropertyEntry(
    countryCode = "zzz",
    income = Some(
  ...
  "reads" should {
    "read from JSON" when {
      "valid JSON is provided" in {
        mtdJson.as[Def1_Create_CreateForeignNonFhlPropertyEntry] shouldBe model
      }
    }
  }
