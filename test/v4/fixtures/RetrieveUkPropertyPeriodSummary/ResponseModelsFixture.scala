...
import v4.models.response.retrieveUkPropertyPeriodSummary._
...
trait ResponseModelsFixture {
  ...
  val fhlRentARoomIncomeModel: RentARoomIncome = RentARoomIncome(
    rentsReceived = Some(1.13)
  )

  val fhlPropertyIncomeModel: FhlPropertyIncome = FhlPropertyIncome(
    periodAmount = Some(1.11),
    taxDeducted = Some(1.12),
    rentARoom = Some(fhlRentARoomIncomeModel)
  )

  val fhlRentARoomExpensesModel: RentARoomExpenses = RentARoomExpenses(
    amountClaimed = Some(2.19)
  )

  val fhlPropertyExpensesModel: FhlPropertyExpenses = FhlPropertyExpenses(
  ...
  val ukFhlPropertyModel: UkFhlProperty = UkFhlProperty(
    income = Some(fhlPropertyIncomeModel),
    expenses = Some(fhlPropertyExpensesModel)
  )

  val ukNonFhlRentARoomIncomeModel: RentARoomIncome = RentARoomIncome(
    rentsReceived = Some(3.16)
  )

  val ukNonFhlIncomeModel: NonFhlPropertyIncome = NonFhlPropertyIncome(
  ...
  val ukNonFhlRentARoomExpensesModel: RentARoomExpenses = RentARoomExpenses(
    amountClaimed = Some(4.21)
  )

  val ukNonFhlExpensesModel: NonFhlPropertyExpenses = NonFhlPropertyExpenses(
  ...
  val ukNonFhlPropertyModel: UkNonFhlProperty = UkNonFhlProperty(
