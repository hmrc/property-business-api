...
import v5.amendForeignPropertyPeriodSummary.def2.model.request.def2_foreignPropertyEntry.{
...
class Def2_AmendForeignNonFhlPropertyEntrySpec extends UnitSpec {
  ...
  val model: Def2_AmendForeignNonFhlPropertyEntry = Def2_AmendForeignNonFhlPropertyEntry(
    countryCode = "zzz",
    income = Some(
      Def2_ForeignNonFhlPropertyIncome(
  ...
  "reads" should {
    "read from JSON" when {
      "valid JSON is provided" in {
        mtdJson.as[Def2_AmendForeignNonFhlPropertyEntry] shouldBe model
      }
    }
  }
