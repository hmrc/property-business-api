...
package v5.createUkPropertyPeriodSummary.def2.model.request

import play.api.libs.json.Json
import shared.utils.UnitSpec
import v5.createUkPropertyPeriodSummary.def2.model.request.def2_ukFhlProperty._
import v5.createUkPropertyPeriodSummary.def2.model.request.def2_ukNonFhlProperty._
import v5.createUkPropertyPeriodSummary.def2.model.request.def2_ukPropertyRentARoom._
...
class CreateUkPropertyPeriodSummaryRequestBodySpec extends UnitSpec {
  ...
  private val requestBody =
    Def2_CreateUkPropertyPeriodSummaryRequestBody(
      "2020-01-01",
      "2020-01-31",
      Some(
        Def2_Create_UkFhlProperty(
          Some(
            Def2_Create_UkFhlPropertyIncome(
              Some(5000.99),
              Some(3123.21),
              Some(Def2_Create_UkPropertyIncomeRentARoom(
                Some(532.12)
              ))
            )),
          Some(Def2_Create_UkFhlPropertyExpenses(
            Some(3123.21),
            Some(928.42),
            Some(842.99),
            Some(8831.12),
            ...
            Some(999.99),
            Some(974.47),
            Some(Def2_Create_UkPropertyExpensesRentARoom(
              Some(8842.43)
            ))
          ))
        )),
  ...
  "reads" when {
    "passed a valid JSON" should {
      "return a valid model" in {
        mtdJson.as[Def2_CreateUkPropertyPeriodSummaryRequestBody] shouldBe requestBody
      }
    }
  }
