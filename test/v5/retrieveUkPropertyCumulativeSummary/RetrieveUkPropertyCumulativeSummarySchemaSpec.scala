...
class RetrieveUkPropertyCumulativeSummarySchemaSpec extends UnitSpec with ScalaCheckDrivenPropertyChecks with TaxYearPropertyCheckSupport {
    ...
    "a correctly formatted tax year is supplied" must {
      "disallow tax years prior to 2025-26 and return RuleTaxYearNotSupportedError" in {
        forTaxYearsBefore(TaxYear.fromMtd("2025-26")) { taxYear =>
          RetrieveUkPropertyCumulativeSummarySchema.schemaFor(taxYear.asMtd) shouldBe Invalid(Seq(RuleTaxYearNotSupportedError))
        }
      }

      "use Def1 for tax years 2025-26 onwards" in {
        forTaxYearsFrom(TaxYear.fromMtd("2025-26")) { taxYear =>
          RetrieveUkPropertyCumulativeSummarySchema.schemaFor(taxYear.asMtd) shouldBe Valid(RetrieveUkPropertyCumulativeSummarySchema.Def1)
        }
      }
    }
    ...
    "a badly formatted tax year is supplied" must {
      "the tax year format is invalid" must {
        "return a TaxYearFormatError" in {
          RetrieveUkPropertyCumulativeSummarySchema.schemaFor("NotATaxYear") shouldBe Invalid(Seq(TaxYearFormatError))
        }
      }

      "the tax year range is invalid" must {
        "return a RuleTaxYearRangeInvalidError" in {
          RetrieveUkPropertyCumulativeSummarySchema.schemaFor("2020-99") shouldBe Invalid(Seq(RuleTaxYearRangeInvalidError))
        }
      }
    }
  }
