...
class UkPropertySpec extends UnitSpec with Def1_RetrieveUkPropertyCumulativeSummaryFixture {

  "NonUkFhlProperty" when {
    val downstreamJson: JsValue = (fullDownstreamJson \ "ukOtherProperty").get
    val mtdJson: JsValue        = (fullMtdJson \ "ukProperty").get

    "read from valid JSON" should {
      "return the parsed object" in {
        downstreamJson.as[UkProperty] shouldBe ukProperty
      }
    }

    "written JSON" should {
      "return the expected JSON" in {
        Json.toJson(ukProperty) shouldBe mtdJson
      }
    }
  }
