...
package v6.amendForeignPropertyPeriodSummary.def2.model.request.foreignPropertyEntry

import play.api.libs.json.{JsValue, Json}
import shared.utils.UnitSpec
import v6.amendForeignPropertyPeriodSummary.def2.model.request.def2_foreignPropertyEntry.Def2_AmendForeignNonFhlPropertyExpenses

class Def2_AmendForeignNonFhlPropertyExpensesSpec extends UnitSpec {
  ...
  val model: Def2_AmendForeignNonFhlPropertyExpenses = Def2_AmendForeignNonFhlPropertyExpenses(
  ...
  "reads" should {
    "read from JSON" when {
      "valid JSON is provided" in {
        mtdJson.as[Def2_AmendForeignNonFhlPropertyExpenses] shouldBe model
      }
    }
  }
