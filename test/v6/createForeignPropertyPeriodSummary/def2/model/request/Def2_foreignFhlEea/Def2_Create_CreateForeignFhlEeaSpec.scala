...
class Def2_Create_CreateForeignFhlEeaSpec extends UnitSpec {
  ...
  private val model = Def2_Create_CreateForeignFhlEea(
    income = Some(Def2_Create_ForeignFhlEeaIncome(rentAmount = Some(567.83))),
    expenses = Some(
  ...
  "reads" should {
    "read from JSON" when {
      "valid JSON is provided" in {
        mtdJson.as[Def2_Create_CreateForeignFhlEea] shouldBe model
      }
    }
  }
