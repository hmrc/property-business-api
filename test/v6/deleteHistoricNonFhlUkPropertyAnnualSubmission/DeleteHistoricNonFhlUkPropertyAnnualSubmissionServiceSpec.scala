...
class DeleteHistoricNonFhlUkPropertyAnnualSubmissionServiceSpec extends ServiceSpec {
  ...
  "service" when {
    "service call successful" should {
      "return mapped result" in new Test {
        MockedDeleteHistoricNonFhlUkPropertyAnnualSubmissionConnector
  ...
  trait Test extends MockDeleteHistoricNonFhlUkPropertyAnnualSubmissionConnector {
    implicit protected val hc: HeaderCarrier              = HeaderCarrier()
    implicit protected val logContext: EndpointLogContext = EndpointLogContext("c", "ep")

    protected val service = new DeleteHistoricNonFhlUkPropertyAnnualSubmissionService(
      connector = mockDeleteHistoricNonFhlUkPropertyAnnualSubmissionConnector
    )

    protected val requestData: DeleteHistoricNonFhlUkPropertyAnnualSubmissionRequestData =
      Def1_DeleteHistoricNonFhlUkPropertyAnnualSubmissionRequestData(nino, taxYear)
