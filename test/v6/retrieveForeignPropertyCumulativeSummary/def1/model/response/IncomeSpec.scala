...
class IncomeSpec extends UnitSpec with Def1_RetrieveForeignPropertyCumulativeSummaryFixture {

  "Income" when {
    val downstreamJson: JsValue = ((fullDownstreamJson \ "foreignProperty")(0) \ "income").get
    val mtdJson: JsValue        = ((fullMtdJson \ "foreignProperty")(0) \ "income").get

    "read from valid JSON" should {
      "return the parsed object" in {
        downstreamJson.as[Income] shouldBe income
      }
    }

    "written JSON" should {
      "return the expected JSON" in {
        Json.toJson(income) shouldBe mtdJson
      }
    }
  }
