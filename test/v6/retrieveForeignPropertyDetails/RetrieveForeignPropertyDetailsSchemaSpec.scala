...
class RetrieveForeignPropertyDetailsSchemaSpec extends UnitSpec with ScalaCheckDrivenPropertyChecks with TaxYearPropertyCheckSupport {
    ...
    "a correctly formatted tax year is supplied" must {
      "disallow tax years prior to 2026-27 and return RuleTaxYearNotSupportedError" in {
        forTaxYearsBefore(TaxYear.fromMtd("2026-27")) { taxYear =>
          RetrieveForeignPropertyDetailsSchema.schemaFor(taxYear.asMtd) shouldBe Invalid(Seq(RuleTaxYearNotSupportedError))
        }
      }
    ...
    "a badly formatted tax year is supplied" must {
      "the tax year format is invalid" must {
        "return a TaxYearFormatError" in {
          RetrieveForeignPropertyDetailsSchema.schemaFor("NotATaxYear") shouldBe Invalid(Seq(TaxYearFormatError))
        }
      }

      "the tax year range is invalid" must {
        "return a RuleTaxYearRangeInvalidError" in {
          RetrieveForeignPropertyDetailsSchema.schemaFor("2020-99") shouldBe Invalid(Seq(RuleTaxYearRangeInvalidError))
        }
      }
    }
  }
