...
class Def1_RetrieveUkPropertyCumulativeSummaryValidatorSpec extends UnitSpec {
  ...
  private def validator(nino: String = validNino, businessId: String = validBusinessId, taxYear: String = validTaxYear) =
    new Def1_RetrieveUkPropertyCumulativeSummaryValidator(nino, businessId, taxYear)

  "validator" should {
    "return the parsed domain object" when {
      "given a valid request" in {
        validator().validateAndWrapResult() shouldBe
          Right(Def1_RetrieveUkPropertyCumulativeSummaryRequestData(parsedNino, parsedBusinessId, parsedTaxYear))
      }
    }
