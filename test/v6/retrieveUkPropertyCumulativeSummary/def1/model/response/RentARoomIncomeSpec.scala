...
class RentARoomIncomeSpec extends UnitSpec with Def1_RetrieveUkPropertyCumulativeSummaryFixture {
  ...
  "RentARoomIncome" when {
    ...
    val mtdJson: JsValue        = (fullMtdJson \ "ukProperty" \ "income" \ "rentARoom").get

    "read from valid JSON" should {
      "return the parsed object" in {
        downstreamJson.as[RentARoomIncome] shouldBe rentARoomIncome
      }
    }

    "written JSON" should {
      "return the expected JSON" in {
        Json.toJson(rentARoomIncome) shouldBe mtdJson
      }
    }
  }
