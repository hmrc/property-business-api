  ...
  "schema lookup" when {
    "a tax year is present" should {
      "use Def1 for tax year 2026-27" in {
        ...
        UpdateForeignPropertyDetailsSchema.schemaFor(taxYear.asMtd) shouldBe Valid(UpdateForeignPropertyDetailsSchema.Def1)
      }
    }

    "an invalid tax year is present" should {
      "return a RuleTaxYearNotSupportedError" in {
        UpdateForeignPropertyDetailsSchema.schemaFor("2025-26") shouldBe Invalid(Seq(RuleTaxYearNotSupportedError))
      }
      "return a TaxYearFormatError" in {
        UpdateForeignPropertyDetailsSchema.schemaFor("NotATaxYear") shouldBe Invalid(Seq(TaxYearFormatError))
      }
      "return a RuleTaxYearRangeInvalidError" in {
        UpdateForeignPropertyDetailsSchema.schemaFor("2020-99") shouldBe Invalid(Seq(RuleTaxYearRangeInvalidError))
      }
    }
  }
